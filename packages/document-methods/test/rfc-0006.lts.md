...
In cases where where we are parsing unknown inputs and validating them we want a
...
definition to allow for flexibility in validation libraries and implementation
while sticking to a standard format.
...
`validate` is an asynchronous method that returns a `Result`: a success carrying the elaborated
...
Common analysis cells shapes will be packaged by us in a `catcolab-analyses` package.
