...
Invalid shapes should be type errors, on `update` as well as `add`. Each case
here has a matching runtime example in [Validation errors](#validation-errors):
...
Each one throws with data from a `FailureResult`, so the examples below mirror
the type errors above case-for-case.
...
#### Phase 3: CatColab scripts execution
