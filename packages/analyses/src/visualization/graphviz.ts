...
export function graphToViz(graph: GraphSpec.Graph, attrs?: GraphvizAttributes): Viz.Graph {
    ...
    return {
        directed: true,
        ...(attrs?.graph !== undefined && { graphAttributes: attrs.graph }),
        ...(attrs?.node !== undefined && { nodeAttributes: attrs.node }),
        ...(attrs?.edge !== undefined && { edgeAttributes: attrs.edge }),
        nodes,
        edges,
    };
}
...
export function parseGraphvizJSON(graphviz: GraphvizJSON.Graph): GraphLayout.Graph {
    ...
    for (const node of (graphviz.objects?.slice(offset) as GraphvizJSON.Node[]) ?? []) {
        ...
        nodes.push({
            ...
            ...(node.class !== undefined && { cssClass: node.class }),
        });
    }
    ...
    for (const edge of graphviz.edges ?? []) {
        ...
        const label = edge.xlabel ?? edge.label;
        const labelPos =
        ...
        edges.push({
            ...
            ...(edge.class !== undefined && { cssClass: edge.class }),
        });
    }
