...
export class Persistence {
    ...
    async saveSnapshot(content: string): Promise<number> {
        return (await queries.newSnapshot.run(
            { content },
            this.pool
        ))[0].id;
    }
    ...
    async newRef(title: string | null): Promise<string> {
        return (await queries.newRef.run(
            { title },
            this.pool
        ))[0].id;
    }
    ...
    async saveRef(refId: string, note: string | null): Promise<number> {
        return (await queries.saveRef.run(
            { refId, note },
            this.pool
        ))[0].id;
    }
    ...
    async allRefs(): Promise<Ref[]> {
        return (await queries.getRefs.run(
            void 1,
            this.pool
        ))
    }
    ...
    async getAutosave(refId: string): Promise<string> {
        return (await queries.getAutosave.run(
            { refId },
            this.pool
        ))[0].content
    }
    ...
    async refMeta(refId: string): Promise<RefMeta> {
        const meta = (await queries.getRefMeta.run(
            { refId },
            this.pool
        ))[0];
        const witnesses = (await queries.getWitnesses.run(
            { refId },
            this.pool
        ));
