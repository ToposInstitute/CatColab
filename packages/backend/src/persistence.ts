...
import { Extern, traverseExterns } from "./links.js";
...
export class Persistence {
  ...
  async autosaveWithExterns(refId: string, doc: any): Promise<void> {
      ...
      await queries.insertNewExterns.run(
        {
          rows: externs.map(e => {
            return {
              fromRef: refId, toRef: e.refId, taxon: e.taxon, via: e.via
            }
          })
        },
        this.pool
