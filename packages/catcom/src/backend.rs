...
use crate::db::{
    default_pg_config, ensure_database_and_user, ensure_postgres_running, pg_config_from_env_file,
    pg_is_ready_at, prompt_pg_config,
};
use crate::env::{ensure_env_files, env_files_exist, read_env_pairs_or_exit, write_env_files};
use crate::process_management::{
    apply_env_defaults, ensure_success_or_exit, exec_or_exit, set_default_var_if_unset,
    set_process_group, spawn_or_exit, status_or_exit,
};
...
pub(crate) fn setup_backend(repo_root: &Path) {
    ...
    if !has_env && pg_is_ready_at("localhost", "5432") {
        ...
        let pg = pg_config_from_env_file(&backend_env);
...
pub(crate) fn spawn_backend(repo_root: &Path) -> Child {
    ...
    let env_vars = read_env_pairs_or_exit(&backend_env_file(repo_root));
    apply_env_defaults(&mut cmd, &env_vars);
    set_default_var_if_unset(&mut cmd, "RUST_LOG", "backend=debug");
...
fn run_migrations(repo_root: &Path) {
    ...
    let env_vars = read_env_pairs_or_exit(&migrator_env_file(repo_root));
    apply_env_defaults(&mut cmd, &env_vars);
