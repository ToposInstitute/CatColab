...
/**
 * The result of a fallible operation: a tagged union structurally identical to
 * the `JsResult` type in our Rust bindings (`catlog-wasm`), which serializes
 * Rust's standard `Result` with `#[serde(tag = "tag", content = "content")]`.
 * Results coming out of the Rust layer can therefore pass through untouched.
 *
 * When a failure is about validation of unknown or user generated input, the
 * `Err` content defaults to an array of [Standard
 * Schema](https://standardschema.dev) {@link Issue}s, so callers can inspect
 * failures vendor-neutrally without importing anything validator-specific.
 */
...
/** Test the structural shape of a morphism-cell handle: `kind` is
 * {@link CellKind.Morphism} and it carries a string `id`; the morphism-side
 * counterpart of {@link looksLikeObjectCell}. */
const looksLikeMorphismCell = (value: unknown): boolean =>
    typeof value === "object" &&
    value !== null &&
    (value as { kind?: unknown }).kind === CellKind.Morphism &&
...
const describeValue = (value: unknown): string => {
    ...
    if (looksLikeMorphismCell(value)) {
        return "a morphism cell";
    }
    return typeof value;
};
 ...
 * {@link ValidationError} on failure. Object cells require a `label`; morphism
 * cells require a `label` and `from`/`to` endpoints, each of which may be `null`
 * to record an unset label or endpoint.
 */
export function validateAddArgs(def: ObjectDef | MorphismDef, args: unknown): void {
    if (def.tag === "object") {
        runSchema(type({ label: "string", "+": "ignore" }), args);
        return;
    }
    ...
    for (const key of ["from", "to"] as const) {
        if (!(key in supplied)) {
            ...
            const shape =
                meta?.modality !== undefined ? "an array or null" : "an object cell or null";
...
export function validateUpdateArgs(def: ObjectDef | MorphismDef, args: unknown): void {
    ...
    if ("label" in supplied) {
        fields["label"] = def.tag === "object" ? "string" : "string | null";
    }
    if (def.tag === "morphism") {
        if ("from" in supplied) {
...
/**
 * The schema for one side of a path equation (`lhs`/`rhs`): `null` (an unset
 * side) or an array of morphism cells — the path's steps in composition order.
 * A single `narrow` over `unknown`, like {@link endpointSchema}, so both halves
 * of every failure message stay under our control and legible.
 */
const pathSchema = type("unknown").narrow((value, ctx) => {
    if (value === null) {
        return true;
    }
    if (!Array.isArray(value)) {
        return ctx.reject({
            expected: "an array of morphism cells or null",
            actual: describeValue(value),
        });
    }
    return value.every(
        (element) =>
            looksLikeMorphismCell(element) ||
            ctx.reject({ expected: "a morphism cell", actual: describeValue(element) }),
    );
});

/**
 * Build and run the validator for a path-equation {@link Notebook.add} call,
 * throwing a {@link ValidationError} on failure. An equation requires a `label`
 * and `lhs`/`rhs` sides; each field may be `null` to record an unset label or
 * side.
 */
export function validatePathEquationAddArgs(args: unknown): void {
    // Side keys are *optional* in the schema so an absent one reaches our own
    // missing-field check below (a legible "must be an array of morphism cells
    // or null (was missing)") rather than ArkType's bare "must be present"; see
    // `validateAddArgs`.
    const schema = type({
        label: "string | null",
        "lhs?": pathSchema,
    ...
    for (const key of ["lhs", "rhs"] as const) {
        if (!(key in supplied)) {
            missing.push({
                message: `\`${key}\` must be an array of morphism cells or null (was missing)`,
                path: [key],
            });
        }
    }
...
/**
 * Build and run the validator for a path-equation {@link Update.update} call,
 * throwing a {@link ValidationError} on failure. Like {@link
 * validateUpdateArgs}, an update is *partial*: only the fields actually present
 * in `args` are checked.
 */
export function validatePathEquationUpdateArgs(args: unknown): void {
    const supplied =
        typeof args === "object" && args !== null ? (args as Record<string, unknown>) : {};
