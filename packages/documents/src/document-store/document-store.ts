...
export interface DocumentStore<Handle, Version = unknown> {
    ...
    // List the handles of all child documents that link back to the document
    // at `handle`, that is, all instances of it. Matching is by document id,
    // so children of any version of the document are included. The order of
    // the results is unspecified.
    listChildren(handle: Handle): Promise<ReadonlyArray<Handle>>;
