...
export interface DocumentStore<Handle, Version = unknown> {
    ...
    // List the handles of all documents that depend on the document at
    // `handle`, that is, all instances of it. Matching is by document id, so
    // documents depending on any version of the document are included. The
    // order of the results is unspecified.
    listUsedBy(handle: Handle): Promise<ReadonlyArray<Handle>>;
