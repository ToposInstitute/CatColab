...
import type { Instance, Notebook } from "catcolab-documents";
...
export function projectDataScript(doc: DataScriptDocument): DataScriptProjection {
    ...
    for (const { entity, row } of rows) {
        ...
        for (const { cell, kind } of morphisms) {
            ...
            if (kind === "mapping") {
                const targetId = typeof value === "object" && value !== null ? value.id : undefined;
                if (typeof targetId === "string") {
                    datoms.push([
                        eid,
                        queryAttribute,
                        rowEids.get(targetId) ?? danglingEid(targetId),
                    ]);
                }
            } else if (
                typeof value === "string" ||
