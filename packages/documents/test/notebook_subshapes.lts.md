...
```ts
import { createBinder, defineObject, defineShape, type Notebook } from "catcolab-documents";
...
function renamePlaces(notebook: Notebook): string[] {
    for (const place of notebook.cellsOf(Place)) {
        place.update({ label: place.label.toUpperCase() });
    }
    notebook.add(Place, { label: "new" });
    return notebook.cellsOf(Place).map((place) => place.label);
}
```
...
const a = net.add(Place, { label: "a" });
const c = net.add(Place, { label: "c" });
const transition = net.add(Transition, { label: "fires", from: [a], to: [c] });
```
...
like `.label` off it directly is a type error.
...
for (const cell of net.cells()) {
    if (cell.kind === CellKind.Morphism) {
        // @ts-expect-error `from` is a single cell or a list; a list has no `label`.
        console.log(cell.from.label);
    }
}
```
...
A list editor written for places types its prop as `NotebookCell<typeof Place>[]`. A schema
...
function renderPlaces(places: NotebookCell<typeof Place>[]): string {
...
// @ts-expect-error A `NotebookCell<typeof Place>[]` prop rejects schema `Entity` cells.
renderPlaces([person]);
```

### An update with a foreign cell

A transition's `from` is `NotebookCell<typeof Place>[]`, and `update` constrains the new value the
...
const schema = await binder.createNotebook(SimpleSchema, { title: "Schema" });
const entity = schema.add(Entity, { label: "Person" });

// @ts-expect-error A transition's `from` is `NotebookCell<typeof Place>[]`; an `Entity` cell is rejected.
...
await binder.createNotebook(PlacesShape, { title: "Nope" });
```
