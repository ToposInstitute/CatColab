 ...
 *   4. Selecting one edits the typed notebook; `Notebook.onValidate` — wired
 *      to a signal — re-validates reactively into a fresh elaborated model.
 */
...
// Test hooks: a real consumer keeps the validation signal and the derived
...
describe("simple-schema completions consumer", () => {
    test("catcolab-documents, elaborated-model completions", async () => {
        ...
        const attr = notebook.add(Attr, { label: "name", from: person, to: str });
        ...
        // re-elaborates when anything the validation depends on changes (a
        // formal-cell edit, but not, say, a rich-text comment — equivalent
        // results are not re-delivered), and the derived `model` accessor
        // exposes its elaborated `DblModel`.
        function Consumer(props: { text: string }) {
            const [validation, setValidation] = createSignal<ModelValidationResult>();
            notebook.onValidate(setValidation);
            ...
            // generator's id is its cell's id); the reactive observer above
            // re-validates into a fresh elaborated model.
            const onSelect = (id: QualifiedName) => {
                const result = notebook.get(AttrType, id);
                if (result.tag === "Ok") {
                    attr.update({ to: result.content });
                }
            };

            // Test hooks only: expose the signal and model so the test can
