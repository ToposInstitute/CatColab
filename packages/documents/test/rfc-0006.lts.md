...
In CatColab's dual-language architecture, notebooks and other documents are
manipulated by the frontend, written in TypeScript, and are also processed at
the entry point into the categorical core (`catlog`), written in Rust. The data
types for documents must therefore be available in both programming languages.
We define these data types in our Rust crate `document-types`, then
automatically export them to TypeScript type definitions using
[`tsify`](https://github.com/madonoharu/tsify). This approach has the virtue
...
manipulating JSON. Such direct manipulation is often not very ergonomic, and it
unnecessarily entangles UI components with the underlying data model. At the
time of this writing, modest progress has been made toward a document API by
extracting the existing helper methods, mainly to do with generic notebook
manipulation, into a new TypeScript package called `document-methods`. This
preliminary work has not meaningfully changed the system design.

We must also qualify what is meant by "type safety". A distinctive feature of
CatColab is embracing a range of domain-specific logics. Because the document
types defining a model notebook encompass models of _any_ theory, code intended
to manipulate models of an arbitrary theory is type safe, whereas code intended
to manipulate models of a _particular_ theory is not type safe. For example,
TypeScript code implementing analyses for models of a particular theory, such as
the bespoke visualizations of Petri nets or stock-flow diagrams, is not fully
type safe.

The aim of this RFC is to design a TypeScript API for traversing and
editing documents, supporting three main use cases:

1. Internal use in the CatColab frontend
2. Other programmatic use from JavaScript and TypeScript
3. Users instructing LLMs

The API should support type-safe processing of documents at different levels of
genericity, particularly the extreme cases where the logic is arbitrary or is
fixed. This RFC will focus on notebook documents, since notebooks are the only
kind of document currently in use, but the API should be extensible to other kinds
of documents in the future, such as instance documents containing tabular data.

## Technical approach

The design of CatColab faces a recurring tension between genericity and
specificity. Here, on the one hand, we prefer that as little code as possible be
required to add a new theory to the frontend. Ideally, binding a theory to the
frontend will be fully declarative, i.e., it will be given by _data_ rather than
code. On the other hand, we'd like each theory available in the frontend to be
accompanied by an ergonomic and type-safe API for its models and instances.

Let's first review the current mechanism for extending the frontend with a new
theory. To add a theory, one calls a method on `TheoryLibrary` and passes a
large JSON object that configures, among other things, how each object and
morphism type in the theory will be labeled and displayed in the frontend. This
specification is indeed declarative (although that is not currently exploited,
say by a plugin system, as all the JSON constants are defined explicitly in the
CatColab standard library).

This RFC proposes to decompose that monolithic specification into a series of
calls into a more granular API. New methods like `defineObject`,
`defineMorphism`, and `defineShape` return constants that are used to tag model
manipulations for type safety. For example, adding the theory of Petri nets to
the frontend will involve making the definition `export const Place =
defineObject(...)`. Then calling `notebook.add(Place, ...)` on a Petri net
notebook will work as expected, whereas making that call on a schema notebook
will yield an error.

While this proposal means that adding a theory to the frontend will require
writing code, not just supplying data, it has the significant advantages that:

1. it requires no more typing (of data or code) than before, yet
2. it gives for free a well-typed API specialized to models of the theory.

Moreover, this RFC introduces a flexible notion of "shape" that enables writing
type-safe code at intermediate levels of genericity, e.g., for any models with
morphisms whose domain and codomain are lists of basic objects.
...
In cases where functions take in unknown and user-generated inputs, we want a
...
we can use the path field to point at a specific object or morphism. If we have
the need, we can extend this in the future, e.g. by adding a `uuid` field to the
...
changes to the notebook's formal content affect the validation result.
...
The `Binder` abstraction exists to keep the API consistent while being able to
...
import type { Document } from "catcolab-document-types";
import { FakeBackend } from "documents/test/fake_backend";
...
`supportsInstances` indicates that the notebook supports diagrams and tabular
instances. Its `tableObjects` are the declared object definitions that become
tables. When included, the `.Diagram` shape is generated for the logic and the
shape can be passed to `createInstance`. See [Diagram notebooks](#diagram-notebooks)
for using `.Diagram`.
...
`cellsOf` also accepts a `Shape`, as an alternative to a cell definition, to
filter by. `cellsOf` narrows the cell shape type in a similar way to `supports`.
...
will add an `editorRef` field to the RichText cell that our editor component can
make use of.
...
An instance is edited through its tables.
...
const personTable = instance.tables.find((table) => table.id === person.id);
const companyTable = instance.tables.find((table) => table.id === company.id);
if (!personTable || !companyTable) {
    throw new Error("Expected instance tables");
}
const acme = companyTable.addRow();
personTable.addRow({ name: "Alice", employer: acme });
personTable.addRow({ name: "Bob", employer: acme });
```

A table's rows expose values as a `fields` array aligned with its headers.

```ts
for (const row of personTable.rows) {
    const field = row.fields[personTable.headers.findIndex((header) => header.id === name.id)];
    console.log(field?.tag === "String" ? field.content.value : "");
}
```
...
Our API could in principle allow us to define logic shapes as notebooks of `SimpleSchema`. Here is an illustrative definition of `CausalLoop` as a notebook.
...
Instances of `CausalLoop` would then be akin to notebooks of the actual `CausalLoop` logic.

```ts
const predatorPreyDoc = await binder.createInstance(CausalLoop, { title: "Predator-prey" });
const predatorPreyValidation = await predatorPreyDoc.validate();
if (predatorPreyValidation.tag !== "Ok") {
    throw new Error("Instance failed to validate");
}
const predatorPrey = predatorPreyValidation.content.instance;

const variableTable = predatorPrey.tables.find((table) => table.id === Variable.id);
const positiveTable = predatorPrey.tables.find((table) => table.id === PositiveLink.id);
const negativeTable = predatorPrey.tables.find((table) => table.id === NegativeLink.id);
if (!variableTable || !positiveTable || !negativeTable) {
    throw new Error("Expected instance tables");
}
const foxes = variableTable.addRow({ name: "Foxes" });
const rabbits = variableTable.addRow({ name: "Rabbits" });

positiveTable.addRow({ from: rabbits, to: foxes });
negativeTable.addRow({ from: foxes, to: rabbits });
```

To use this flexibility for defining logics through notebooks in practice a lot of details would still need to be worked out of course.
...
To implement this, we can extend the `DocumentStore` to have a `Version` parameter and some added methods.
...
1. **Test suite for model notebooks**
    ...
    feature details and edge cases. If possible, we should implement
    property-based tests that show our new API will not cause document corruption.

2. **Package for model notebooks**

    A package that meets the spec outlined in the test suite. We will not
...
3. **CatColab script execution**
    ...
    interface, for a user to write JavaScript that makes use of this API to create
...
6. **New features use the API & port existing features**
    ...
    Once we have proven the stability and usefulness of the new API, we should
