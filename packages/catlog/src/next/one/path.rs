...
use super::super::zero::graph::Graph;
use super::super::zero::set::*;
use crate::validate;
...
impl<V, E> Path<V, E> {
    ...
    */
        ...
        G: Graph<V = V, E = E>,
    {
        match self {
            Path::Id(v) => v.clone(),
            Path::Seq(edges) => graph.src().ap(edges.first()),
        }
    }
    ...
    */
        ...
        G: Graph<V = V, E = E>,
    {
        match self {
            Path::Id(v) => v.clone(),
            Path::Seq(edges) => graph.tgt().ap(edges.last()),
        }
    }
