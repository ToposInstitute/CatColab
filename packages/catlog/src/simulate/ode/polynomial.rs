    ...
    Exp: Ord,
{
    ...
    /// Converts to equations as Latex strings, after applying some `f : &QualifiedName -> String`
    /// to each of the variables and coefficients. This is for frontend functionality, where we
    /// do not want to display UUIDs directly but instead look them up in the model namespace. For more
    /// details, see `catlog-wasm::src::latex` where we use `to_latex_equations_with_map` with argument
    /// `catlog-wasm::src::latex_names`.
    pub fn to_latex_equations_with_map<F: Fn(&QualifiedName) -> String>(
        &self,
        f: F,
    ) -> LatexEquations
    where
        // It would be much cleaner to only implement `to_latex_equations_with_map` in the case where
        // `Var = QualifiedName` and `Coef = Parameter<P: ODEParameter>`, but I believe that Rust only
        // allows one implementation per trait+type pair, irrelevant of generics.
        Var: Display + ToLatexWithMap,
        Coef: Display + ToLatexWithMap + DisplayCoef + Clone + PartialEq + One + Neg<Output = Coef>,
        Exp: Display + ToLatex + PartialEq + One,
    {
