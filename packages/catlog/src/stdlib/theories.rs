...
pub fn th_schema_maybe() -> ModalDblTheory<NonUnital> {
    ...
    th.add_mor_type(name("MaybeAttr"), m_entity.clone(), m_attr.apply(Modality::Maybe()));
    th.add_mor_type(name("Rel"), m_entity.clone(), m_entity.clone());
    th.add_mor_type(name("MaybeRel"), m_entity.clone(), m_entity.apply(Modality::Maybe()));
    th
}
