...
use crate::one::{
    Category, FgCategory, QualifiedPath,
    path::{Path, PathEq},
};
use crate::zero::{Namespace, QualifiedName};
use crate::{
    dbl::{
        discrete::{self, DiscreteDblModelMapping},
        discrete_tabulator,
        modal::{self, ModalDblModel, ModalDblModelMapping, ModalMorType, ModalOb, ModalObType},
        model::{DblModel, DblModelPrinter, DiscreteDblModel, MutDblModel},
        model_diagram::{DblModelDiagram, DblModelDiagramType},
...
pub fn diagram_from_diag(
    ...
    th: &TheoryDef,
    diag: &Diag,
) -> Result<(DblModelDiagramType, Namespace, Vec<(TmS, TmS)>), String> {
    match th {
        TheoryDef::Discrete(theory) => {
            ...
            let DiagramGenerator { domain, mapping, equations, .. } = generator;
            Ok((
                DblModelDiagramType::Discrete(DblModelDiagram(mapping, domain)),
                namespace,
                equations,
            ))
        }
        TheoryDef::ModalUnital(theory) => {
            let mut generator = DiagramGenerator {
                eval: Evaluator::empty(toplevel),
                codomain_ty: diag.model.clone(),
                domain: modal::ModalDblModel::new(theory.clone()),
                mapping: modal::ModalDblModelMapping::default(),
                equations: vec![],
            };
            let namespace = generator.generate(&diag.body_val)?;
            let DiagramGenerator { domain, mapping, equations, .. } = generator;
...
struct DiagramGenerator<'a, M: DblModel + MutDblModel + FgCategory, Mapping: MutDblModelMapping> {
    eval: Evaluator<'a>,
    codomain_ty: TyV,
    domain: M,
    mapping: Mapping,
...
impl DiagramGenerator<'_, DiscreteDblModel, DiscreteDblModelMapping> {
    ...
    fn extract(
        ...
        match &**ty {
            ...
            TyV_::Object(_) | TyV_::Sing(_, _) | TyV_::Unit | TyV_::Meta(_) => Ok(None),
        }
    }
}

impl DiagramGenerator<'_, ModalDblModel<Unital>, ModalDblModelMapping> {
    fn generate(&mut self, body_ty: &TyV) -> Result<Namespace, String> {
        let (self_n, eval_with_self) = self.eval.bind_self(body_ty.clone());
        self.eval = eval_with_self;
        let self_val = self.eval.eta_neu(&self_n, body_ty);
    ...
    fn extract(
        ...
        match &**ty {
            ...
            TyV_::Over(path) => {
                ...
                let ot: ModalObType = ot.clone().try_into().map_err(|_| {
                    "@over codomain object type is not valid for a discrete theory".to_string()
                })?;
                let qname: QualifiedName = prefix.into();
                self.domain.add_ob(qname.clone(), ot.into());
                let target: QualifiedName =
                    path.iter().map(|(seg, _)| *seg).collect::<Vec<_>>().into();
                self.mapping.assign_ob(qname, target.into());
                Ok(None)
            }
            TyV_::Morphism(mt, dom, cod) => {
                // Augmentation produces lift morphisms whose dom and cod
                ...
                if let Some(last) = prefix.last()
                    && let Some(cod_mor) = parse_lift_morphism_name(last)
                {
                    let target: modal::ModalMor = QualifiedName::single(cod_mor).into();
                    self.mapping.assign_mor(qname, target);
                }
                Ok(None)
            }
...
mod tests {
    ...
    fn elaborate_to_toplevel(src: &str) -> Toplevel {
        ...
        if reporter.errored() {
            let source_info = SourceInfo::new(None, src);
            panic!("{}", source_info.extract_report_to_string(reporter.clone()));
        }
...
"#;
        ...
        match model_diag {
            ...
            DblModelDiagramType::ModalUnital(_) => {
                panic!("We should not here!")
            }
        }
    }
...
"#;
    ...
    fn glueing_modal_diagrams() {
        ...
        match model_diag {
            ...
            DblModelDiagramType::ModalUnital(DblModelDiagram(mapping, domain)) => {
                let e_qname: QualifiedName = vec![name_seg("a")].into();
                let entity_ot = modal::ModeApp::new(name("Object"));
                let e_target: QualifiedName = vec![name_seg("Form0")].into();
                assert!(domain.has_ob(&e_qname.clone().into()));
                assert_eq!(domain.ob_generator_type(&e_qname), entity_ot);
