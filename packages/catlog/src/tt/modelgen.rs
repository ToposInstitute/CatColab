...
use crate::dbl::{
    ...
    model_morphism::{DblModelMapping, MutDblModelMapping},
...
pub fn diagram_from_diag(
    ...
    th: &TheoryDef,
    diag: &Diag,
) -> Result<(DblModelDiagramType, Namespace), String> {
    match th {
        TheoryDef::Discrete(theory) => {
            ...
            Ok((DblModelDiagramType::Discrete(DblModelDiagram(mapping, domain)), namespace))
        }
        TheoryDef::ModalUnital(theory) => {
            let mut generator = DiagramGenerator {
                eval: Evaluator::empty(toplevel),
            ...
            let namespace = generator.generate(&diag.body_val)?;
            let DiagramGenerator { domain, mapping, .. } = generator;
            Ok((DblModelDiagramType::ModalUnital(DblModelDiagram(mapping, domain)), namespace))
        }
        _ => Err("!".into()),
    }
}
