...
/// A helper to display an optional "over" annotation: prints " over {inner}"
/// when the inner value is non-empty, and nothing otherwise.
pub struct DHOver<'a, T: std::fmt::Display>(pub &'a T);
impl<T: std::fmt::Display> std::fmt::Display for DHOver<'_, T> {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
...
#[derive(Display)]
#[display("{}", if let Some(v) = _0 {v.to_string()} else {"ε".to_string()})]
/// A helper to display an optional value.
pub struct DHOption<'a, T: std::fmt::Display>(pub Option<&'a T>);
