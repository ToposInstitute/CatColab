...
use crate::mtt::display_helpers::{DHBindings, DHList, DHOver, DHProArrowComposite, DHTuple};
...
pub enum Decl {
    ...
    #[display("const {name} : {dom} -> {cod}{};", DHOver(over))]
    ...
    #[display("fn {name}({binder}) -> {codomain}{} {{\n{}\n}}", DHOver(over), indent(&body.to_string(), "  "))]
...
pub enum Expression {
    ...
    #[display("{subject}: {domain} -> {codomain}{}", DHOver(over))]
