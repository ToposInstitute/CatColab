//! A unified approach to handling modality applications on various kinds of
//! theory data.

use crate::mtt::theory::{Theory, TheoryObject, TheoryProArrow, TheoryVerticalArrow};
...
pub trait ModalDepth: Sized + Clone {
    ...
    /// [Self::increment_modal_depth] or [Self::decrement_modal_depth] would fail in the
...
impl<T: Theory> ModalDepth for TheoryObject<T> {
    ...
    fn increment_modal_depth(&self) -> Option<Self> {
        ...
        Some(TheoryObject::ModalApplication(Box::new(self.clone())))
    }
...
impl<T: Theory> ModalDepth for TheoryVerticalArrow<T> {
    fn modal_depth(&self) -> usize {
        match self {
            TheoryVerticalArrow::Generator { .. } | TheoryVerticalArrow::ModalStructureMap(_) => 0,
            TheoryVerticalArrow::ModalApplication(on) => 1 + on.modal_depth(),
        }
    }
    ...
    fn increment_modal_depth(&self) -> Option<Self> {
        ...
        Some(TheoryVerticalArrow::ModalApplication(Box::new(self.clone())))
    }

    fn decrement_modal_depth(&self) -> Option<Self> {
        match self {
            TheoryVerticalArrow::ModalApplication(on) => Some(*on.clone()),
            _ => None,
        }
    }
}
...
impl<T: Theory> ModalDepth for TheoryProArrow<T> {
    ...
    fn increment_modal_depth(&self) -> Option<Self> {
        ...
        Some(TheoryProArrow::ModalApplication(Box::new(self.clone())))
    }
