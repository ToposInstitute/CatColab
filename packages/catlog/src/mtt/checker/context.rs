//! Contexts and related types for holding checking and derivation state of MTT
//! programmes.
...
#[derive(Display)]
#[display("{derivation} with scope {}", DHMap(domain_scope))]
...
pub struct DefinitionEntry<T: Theory> {
    ...
    /// The variable scope introduced by the definition's checked domain binder.
    pub domain_scope: Scope<T>,
...
impl<T: Theory> ModelEntry<T> {
    pub fn lookup_generating_object(&self, name: &String) -> Result<&ObjectEntry<T>, EContext> {
        self.object_generators.get(name).ok_or(EContext::Unbound(name.clone()))
    }
}

impl<T: Theory> ModelEntry<T> {
    fn contains_name(&self, name: &String) -> bool {
        self.object_generators.contains_key(name)
            || self.pro_arrow_generators.contains_key(name)
    ...
    pub fn lookup_generating_pro_arrow_entry(
        ...
        self.pro_arrow_generators.get(name).ok_or(EContext::Unbound(name.clone()))
    }
...
/// The object-safe interface to a checked [ModelEntry] whose theory parameter
/// has been existentially hidden.
trait ErasedModelEntry: Any {
    fn theory_name(&self) -> &'static str;
    fn as_any(&self) -> &dyn Any;
}

impl<T: Theory> ErasedModelEntry for ModelEntry<T> {
    fn theory_name(&self) -> &'static str {
        T::NAME
    }
...
/// A checked model entry with an existentially hidden theory parameter.
pub struct CheckedModel(Box<dyn ErasedModelEntry>);

impl CheckedModel {
    /// Hide the theory parameter of a checked model entry.
    pub fn new<T: Theory>(entry: ModelEntry<T>) -> Self {
        Self(Box::new(entry))
    }

    /// The name of the theory indexing the checked model entry.
    pub fn theory_name(&self) -> &'static str {
        self.0.theory_name()
    }

    /// Recover the model entry when it is indexed by `T`.
    pub fn downcast_ref<T: Theory>(&self) -> Option<&ModelEntry<T>> {
        self.0.as_any().downcast_ref::<ModelEntry<T>>()
    }
}

#[derive(Default)]
/// The checked models accumulated while processing a programme.
pub struct ProgrammeContext {
    /// Checked models indexed by their model names.
    pub models: HashMap<String, CheckedModel>,
}

impl ProgrammeContext {
    /// Add a checked model to the programme.
    pub fn add_model<T: Theory>(
        &mut self,
        name: &str,
        model: ModelEntry<T>,
    ) -> Result<(), EContext> {
        if self.models.contains_key(name) {
            return Err(EContext::Redecleration(name.to_string()));
        }
        self.models.insert(name.to_string(), CheckedModel::new(model));
        Ok(())
    }

    /// Look up a checked model by its model name.
    pub fn lookup_model(&self, name: &String) -> Result<&CheckedModel, EContext> {
        self.models.get(name).ok_or(EContext::Unbound(name.clone()))
    }
}
