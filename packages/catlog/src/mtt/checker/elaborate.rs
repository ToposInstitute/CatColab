...
use crate::mtt::{
    ...
    composite::Composite,
    hole::Holy,
    theory::{ListModality, Theory, TheoryObject, TheoryProArrow, TheoryVerticalArrow},
};
...
impl<T: Theory> ModelEntry<T> {
        ...
        match obj {
            ...
            Expression::Juxtaposition { .. } => {
                ...
                if let Expression::Literal(post) = *post {
                    if !T::has_list_modality() || post != <T::ListModality as ListModality>::NAME {
                        return Err(EElaborate::UnknownModality(post));
                    }
    ...
    pub fn elaborate_object_type(&self, obj: &Expression) -> Result<ObjectType<T>, EElaborate> {
        match obj {
            ...
            Expression::Juxtaposition { .. } => {
                ...
                // arrows.
                let mut spine: Vec<TheoryVerticalArrow<T>> = Vec::new();
                while let Expression::Juxtaposition { post, pre } = expr {
                    ...
                    let Some(arr) = T::generating_vertical_arrow_by_name(&fun) else {
                ...
                let function =
                    Composite::try_from(spine).map_err(EElaborate::InvalidTheoryArrowComposite)?;
    ...
    #[tracing::instrument(skip(self), level = "debug", fields(domain = %domain, codomain = %codomain, over = %over))]
    pub fn elaborate_annotation(
        ...
        let domain_object_type = self.elaborate_object_type(domain)?;
        let codomain_object_type = self.elaborate_object_type(codomain)?;

        // If the user supplied an explicit `over`, use it. Otherwise, try to
        // infer the theory pro-arrow from the boundary. Both inference steps
        // (theory objects, then pro-arrow) may fail for annotations whose
        // types involve holes or are otherwise ambiguous; in that case we
        // fall back to an unconstrained hole.
        let pro_arrow = match self.elaborate_theory_pro_arrow(over)? {
            Some(explicit) => explicit,
            None => self
                .infer_theory_object(&domain_object_type)
                .and_then(|t_dom| {
                    let t_cod = self.infer_theory_object(&codomain_object_type)?;
                    self.infer_theory_pro_arrow_by_boundary(&t_dom, &t_cod)
                })
                .ok()
                .unwrap_or_else(|| unconstrained.pro_arrow.clone()),
        };
    ...
    pub fn elaborate_pro_arrow(
        ...
        Ok(ModelGeneratingProArrow { name: name.to_owned(), dom, cod })
    }
