//! Checking and derivation procedures in the context of [ModelEntry]. With the
//! exception of definitions and relations, this module contains the bulk of the
...
use crate::mtt::{
    ...
    checker::{
        context::{
            ...
            ProTermJudgement, ProgrammeContext, RelationEntry,
        },
...
impl<T: Theory> ModelEntry<T> {
    pub fn check_model_ast(
        &mut self,
        model_ast: &Model,
        programme_context: &ProgrammeContext,
    ) -> CheckResult {
        let Model { name: _name, decls, theory: _theory } = model_ast;
        // TODO: name unused?
        // TODO: theory unused?
        for decl in decls.iter() {
            tracing::debug!("checking: {decl}");
            self.check_model_decl(decl, programme_context)?;
            tracing::debug!("  ok");
        }
    ...
    fn check_model_decl(
        &mut self,
        decl: &Decl,
        _programme_context: &ProgrammeContext,
    ) -> CheckResult {
        match decl {
            ...
            Decl::ProArrowGenerator { name, dom, cod, over } => {
                ...
                let over = match over {
                    ...
                    None => {
                        ...
                        match T::generating_pro_arrows_by_boundary(&t_dom, &t_cod) {
                            ProArrowByBoundary::Determined(atom) => atom,
                            ProArrowByBoundary::None => {
                                return Err(EInfer::NoTheoryProArrow(
                            ...
                            ProArrowByBoundary::Ambiguous => {
                                return Err(EInfer::AmbiguousTheoryProArrow(
                                    TheoryProArrow::Generator {
                                        name: "?".to_string(),
                                        dom: t_dom,
                                        cod: t_cod,
            ...
            Decl::Definition { name, binder, codomain, over, body } => {
                ...
                let domain_scope = self.build_domain_scope(
                    &judgement.domain_object_term,
                    &judgement.domain_object_type,
                    &judgement.domain_theory_object,
                )?;
                let pro_term =
                    self.elaborate_and_check_pro_term(body, &judgement, &domain_scope)?;
                self.add_definition(
                    name.clone(),
                    DefinitionEntry {
            ...
            Decl::Relation { name, binder, codomain, over, lhs, rhs } => {
                ...
                let lhs_pro_term =
                    self.elaborate_and_check_pro_term(lhs, &judgement, &domain_scope)?;
                let rhs_pro_term =
                    self.elaborate_and_check_pro_term(rhs, &judgement, &domain_scope)?;
                self.add_relation(
                    name.clone(),
                    RelationEntry {
                        lhs: Derivation {
...
impl<T: Theory> ModelEntry<T> {
            ...
            ObjectType::List(list) => {
                ...
                if list.is_empty() {
                    return Ok(TheoryObject::ModalApplication(Box::new(
                        TheoryObject::unconstrained("theory_object_for_empty_list".to_string()),
                    )));
                }
    ...
    pub fn infer_theory_pro_arrow_by_boundary(
        ...
        match T::pro_arrow_by_boundary(t_dom, t_cod) {
            ProArrowByBoundary::Determined(composite) => Ok(composite),
