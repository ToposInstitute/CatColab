//! Model-level types: objects, arrows, and derivations.

use derive_more::Display;

use crate::mtt::{
    composite::Composite,
    theory::{Boundary, Theory, TheoryObject, TheoryProArrow, TheoryVerticalArrow},
};
...
pub enum ObjectType<T: Theory> {
    ...
    FunctionApplication {
        ...
        function: Composite<TheoryVerticalArrow<T>>,
...
/// A derivation, the main content of the type checker, these are specified in
/// the type theory by deriving judgements of the form Γ | x : X ⊢_P y : Y.
pub enum ProTerm<T: Theory> {
    ...
    /// any change in modal depth ([TheoryVerticalArrow::ModalStructureMap]) and
    /// corresponding [ObjectType] changes in a [ProTerm::CellApplication],
    /// while this node carries the transformation of the domain's variables.
    ///
    /// The `before` and `after` object types record the domain's shape on each
    /// side of the reindex; the modality decides whether `reindex` is
    /// admissible between them. There is no codomain counterpart, as the
    /// pro-term itself is the codomain and is not a variable as such.
    ListReindex {
