...
where
    G::ProE: std::fmt::Debug,
{
    type Ob = G::V;
    type Arr = Path<G::V, G::E>;
    type Pro = G::ProE;
    type Cell = DblTree<G::E, G::ProE, G::Sq>;

    fn has_ob(&self, ob: &Self::Ob) -> bool {
        self.0.has_vertex(ob)
    }
    fn has_arrow(&self, path: &Self::Arr) -> bool {
        path.contained_in(EdgeGraph::ref_cast(&self.0))
    }
    fn has_proarrow(&self, pro: &Self::Pro) -> bool {
        self.0.has_proedge(pro)
    }
    fn has_cell(&self, tree: &Self::Cell) -> bool {
        tree.contained_in(&self.0)
    }

    fn dom(&self, path: &Self::Arr) -> Self::Ob {
        path.src(EdgeGraph::ref_cast(&self.0))
    }
    fn cod(&self, path: &Self::Arr) -> Self::Ob {
        path.tgt(EdgeGraph::ref_cast(&self.0))
    }
    fn src(&self, m: &Self::Pro) -> Self::Ob {
        self.0.src(m)
    }
    ...
    fn compose(&self, path: Path<Self::Ob, Self::Arr>) -> Self::Arr {
        path.flatten_in(EdgeGraph::ref_cast(&self.0))
