...
pub mod WalkingBimodule {
    ...
    The **walking bimodule** is the unital virtual double category freely
    generated by a pair of objects, here called [`Left`](Ob::Left) and
    [`Right`](Ob::Right), and a single proarrow between them, here called
    [`Middle`](Pro::Middle). In fact, this VDC has all composites.
    */
    ...
    /// Type of an object in the walking bimodule.
    #[derive(Clone, Copy, Debug, PartialEq, Eq)]
    pub enum Ob {
        /// Object that is the source of the bimodule.
        Left,
        /// Object that is the target of the bimodule.
        Right,
    }

    /// Type of a proarrow in the walking bimodule.
    #[derive(Clone, Copy, Debug, PartialEq, Eq)]
    pub enum Pro {
        ...
        fn cell_cod(&self, path: &Path<Ob, Pro>) -> Self::Pro {
            match path {
                ...
                Path::Seq(pros) => {
                    ...
                    *path.iter().find(|m| **m == Pro::Middle).unwrap_or_else(|| pros.first())
                }
            }
        }
...
mod tests {
    ...
    fn walking_category() {
        ...
        assert_eq!(vdc.unit_ext(()), Some(Path::empty(())));
    }
