...
impl ModalDblModelDiagram {
    ...
    pub fn infer_missing_from(&mut self, model: &ModalDblModel<Unital>) {
        ...
        for e in domain.mor_generators() {
            ...
            if let Some(x) = domain.get_dom(&e).filter(|x| {
                // TODO no clone/clone
                if let Some(id) = x.clone().clone().generator() {
                    !mapping.0.is_vertex_assigned(&id)
                } else {
                    false
                }
            }) {
                ...
                if let Some(id) = x.clone().generator() {
                    mapping.assign_ob(id.clone(), model.dom(&g));
                }
            }
            if let Some(cod) = domain.get_cod(&e).filter(|x| {
                if let Some(id) = x.clone().clone().generator() {
                    !mapping.0.is_vertex_assigned(&id)
                } else {
                    false
                ...
                if let Some(id) = cod.clone().generator() {
                    mapping.assign_ob(id.clone(), model.cod(&g));
                }
            }
        }
    }
...
mod tests {
    ...
    fn validate_modal_model_diagram() {
        ...
        domain.add_ob(name("Form0"), ob_type.clone());
        ...
        let diagram = DblModelDiagram(f, domain.clone());
