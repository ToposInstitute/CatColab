...
pub trait ColumnarDblComputad {
        ...
        Self: Sized,
    {
        ...
        let edge_errors = edge_graph.iter_invalid().map(|err| match err {
            InvalidGraphData::Src(e) => Invalid::Dom(e),
            InvalidGraphData::Tgt(e) => Invalid::Cod(e),
        });

        let proedge_graph = ProedgeGraph::ref_cast(self);
        let proedge_errors = proedge_graph.iter_invalid().map(|err| match err {
...
impl<Cptd: ColumnarDblComputad> DblComputad for Cptd {
    ...
    fn square_cod(&self, α: &Self::Sq) -> Path<Self::V, Self::ProE> {
        self.square_cod_map()
            .apply(α)
            .expect("Codomain of square should be set")
            .clone()
    }
    fn square_src(&self, α: &Self::Sq) -> Path<Self::V, Self::E> {
...
pub struct HashDblComputad<V, E, ProE, Sq> {
    ...
    proedge_set: HashFinSet<ProE>,
    square_set: HashFinSet<Sq>,
    dom_map: HashColumn<E, V>,
    cod_map: HashColumn<E, V>,
    src_map: HashColumn<ProE, V>,
    tgt_map: HashColumn<ProE, V>,
    sq_dom_map: HashColumn<Sq, Path<V, ProE>>,
    sq_cod_map: HashColumn<Sq, Path<V, ProE>>,
    sq_src_map: HashColumn<Sq, Path<V, E>>,
    sq_tgt_map: HashColumn<Sq, Path<V, E>>,
}
...
perform validation.
*/
pub struct DblComputadMorphism<'a, Map, Dom, Cod>(pub &'a Map, pub &'a Dom, pub &'a Cod);

impl<'a, Map, Dom, Cod> DblComputadMorphism<'a, Map, Dom, Cod>
where
    Map: DblComputadMapping,
    ...
    Cod: DblComputad<V = Map::CodV, E = Map::CodE, ProE = Map::CodProE, Sq = Map::CodSq>,
{
    ...
    + use<'a, Map, Dom, Cod> {
        ...
        let edge_hom = GraphMorphism(
            EdgeGraphMapping::ref_cast(mapping),
            EdgeGraph::ref_cast(dom),
            EdgeGraph::ref_cast(cod),
        );
        let edge_errors = edge_hom.iter_invalid().map(|err| match err {
        ...
        let proedge_hom = GraphMorphism(
            ProedgeGraphMapping::ref_cast(mapping),
