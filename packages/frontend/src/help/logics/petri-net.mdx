...
Like ordinary mass-action dynamics, stochastic mass-action dynamics for Petri Nets interpret places in their underlying model as time-varying quantities which transfer mass between one another through transitions, which themselves determine rate coefficients.

The difference here is that the time between reactions is determined by an exponential distribution whose mean is the total reaction rate.
