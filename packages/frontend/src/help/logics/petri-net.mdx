...
Furthermore, these rates are determined at the level of *places*, not *transitions*.

That is, a transition $[A,B]\xrightarrow{T}[X,Y]$ is interpreted as the equations
...
If $\kappa_T^A=\kappa_T^B$ and $\rho_T^C=\rho_T^D$ then this corresponds to the transition having *global* **consumption rate** $\kappa_T$ and **production rate** $\rho_T$; if furthermore $\kappa_T=\rho_T$ then we recover the usual mass-action dynamics.
</HelpAnalysisById>
