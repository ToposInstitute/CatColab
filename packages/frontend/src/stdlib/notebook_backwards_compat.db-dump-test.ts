...
describe("Database dump backward compatibility", () => {
    ...
    const indexedModelAnalyses = modelAnalyses.map((doc, i) => ({
        doc: doc as Record<string, unknown>,
        i,
    }));
    const skiplistedModelAnalyses = indexedModelAnalyses.filter(({ doc }) => {
        const docRefId = (doc._refId as string | undefined) ?? "no-ref-id";
        return skippedIds.has(docRefId);
    });
    const runnableModelAnalyses = indexedModelAnalyses.filter(({ doc }) => {
        const docRefId = (doc._refId as string | undefined) ?? "no-ref-id";
        return !skippedIds.has(docRefId);
    });

    for (const { doc, i } of skiplistedModelAnalyses) {
    ...
    for (const { doc, i } of runnableModelAnalyses) {
        const docName = (doc.name as string | undefined) ?? "unnamed";
        const docRefId = (doc._refId as string | undefined) ?? "no-ref-id";
    ...
    // analysis functions.
    const indexedDiagramAnalyses = diagramAnalyses.map((doc, i) => ({
        doc: doc as Record<string, unknown>,
        i,
    }));
    const skiplistedDiagramAnalyses = indexedDiagramAnalyses.filter(({ doc }) => {
        const docRefId = (doc._refId as string | undefined) ?? "no-ref-id";
        return skippedIds.has(docRefId);
    });
    const runnableDiagramAnalyses = indexedDiagramAnalyses.filter(({ doc }) => {
        const docRefId = (doc._refId as string | undefined) ?? "no-ref-id";
        return !skippedIds.has(docRefId);
    });

    for (const { doc, i } of skiplistedDiagramAnalyses) {
        const docName = (doc.name as string | undefined) ?? "unnamed";
        const docRefId = (doc._refId as string | undefined) ?? "no-ref-id";
        // oxlint-disable-next-line no-disabled-tests
    ...
    for (const { doc, i } of runnableDiagramAnalyses) {
        const docName = (doc.name as string | undefined) ?? "unnamed";
        const docRefId = (doc._refId as string | undefined) ?? "no-ref-id";
        test(`diagram analysis ${i}: "${docName}" [${docRefId}]`, async () => {
            ...
            const diagramJudgments: DiagramJudgment[] = Nb.getFormalContent(
 ...
 */
function runAnalysisCells<S extends { run?: (...args: never[]) => unknown }>(
    migratedAnalysis: Record<string, unknown>,
    getAnalysisSpec: (id: string) => S | undefined,
    runSpec: (spec: S, content: unknown) => unknown,
    theoryId: string,
): void {
