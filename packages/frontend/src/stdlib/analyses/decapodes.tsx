...
export default function Decapodes(props: DiagramAnalysisProps<DecapodesAnalysisContent>) {
	...
	// filter out anonymous variables. this is hacky, since a variable could conceivably be named an integer.
    const variables = (): QualifiedName[] => {
        const diagram = elaboratedDiagram();
        if (!diagram) return [];
        return diagram.obGenerators().filter((id) => {
            const label = diagram.obGeneratorLabel(id)?.join(".") ?? "";
            return !Number.isInteger(Number(label));
        });
    };
