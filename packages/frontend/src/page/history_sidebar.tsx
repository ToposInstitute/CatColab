...
import { HistoryNavigator, type HistoryItem } from "catcolab-ui-components";
...
export function HistorySidebar(props: { refId: string }) {
    const api = useApi();
    const userState = useUserState();

    const docInfo = createMemo(() => userState.documents[props.refId]);
    ...
    const navigate = (snapshotId: string) => {
        ...
        if (!Number.isNaN(id)) {
            void api.rpc.set_current_snapshot.mutate(props.refId, id);
        }
    };
