...
function variables_state(soln::ODESolution, system::DecapodesSystem)
    ...
    Dict(String(uuid2symb[var]) => state_entire_sim(soln, system, uuid2symb[var]) for var ∈ keys(system.plotVariables) )
end
...
function points(system::DecapodesSystem)
    system.geometry.dualmesh[:point]
    # collect(values(system.geometry.dualmesh.subparts.point.m))
end
