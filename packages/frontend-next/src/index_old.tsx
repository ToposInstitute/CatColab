        ...
        <div class="notebook">
            ...
            <ul class="cells">
                <For each={props.notebook.val.order}>
                    {(id: Id) => (
                        <li>
                            <Cell cellId={id} notebook={props.notebook} />
                        </li>
                    )}
                </For>
            </ul>
...
function Shelf(props: { shelf: Var<Shelf> }) {
    ...
    if (props.shelf.val.notebooks.size > 0) {
        setNotebookId(props.shelf.val.notebooks.entries().next().value![0]);
    }
    return (
        <div class="shelf">
            <form>
                ...
                <select
                    ...
                    value={notebookId()}
                    onInput={(ev) => {
                        setNotebookId(ev.target.value);
                    }}
                >
                    <For each={[...props.shelf.val.notebooks.keys()].sort()}>
                        {(notebookId) => (
...
function App() {
    let initShelfSerialized = localStorage.getItem("shelf");
    var initShelf = null;
    if (!initShelfSerialized) {
        initShelf = newShelf();
    } else {
        initShelf = JSON.parse(initShelfSerialized, reviver);
    }
    let [shelf, setShelf] = createSignal(initShelf);
    let updateShelf = (f: (n: Shelf) => void) => {
        let s = structuredClone(shelf());
        f(s);
        setShelf(s);
        localStorage.setItem("shelf", JSON.stringify(s, replacer));
    };
    return (
        <div>
            <Header />
