...
Our Typescript type errors should remain clear and legible.
...
also want separate validation that inputs match our expected schema. We throw
and return validation results with `issues` arrays following [Standard
Schema](https://standardschema.dev) definitions.

We generally follow the principle of:

- If invalid inputs are given at runtime, that would be type errors in Typescript, we will _throw_ validation errors.
- If it is possible to give invalid inputs otherwise then the method should return a `Result` type following the Standard Schema definition.
