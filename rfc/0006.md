---
title: |
    `0006` Notebook API for TypeScript
author: Kaspar Bumke
date: 2026-07-08
toc: true
toc-depth: 3
...
This RFC is tracked in [#1347](https://github.com/ToposInstitute/CatColab/issues/1347).
:::
...
to manipulate models of a *particular* theory is not type safe. For example,
...
specificity. In connecting theories with the frontend, we would prefer, on the
one hand, that as little code as possible be required to add a new theory.
Ideally, binding a theory to the frontend would be fully declarative, i.e.,
would be defined by *data*, not code. On the other hand, we would like each
theory to expose an ergonomic and type-safe TypeScript API for its models and
instances.

Let's briefly review the current mechanism for extending the frontend with a new
theory. To add a theory, one calls a method on `TheoryLibrary` and passes a
constructor that configures, among other things, how each object and morphism
type in the theory will be labeled and displayed in the frontend. Apart from
snippets of code binding the `DblTheory` object exposed by `catlog-wasm`, this
constructor consists of a JSON object defined explicitly as a source-level
constant. So, the existing design has the advantage of being essentially
declarative, and it could be made fully so with some refactoring.

This RFC proposes to decompose the monolithic binding of a theory into a series
of calls into a more granular API. New methods like `defineObject`,
...
writing code, rather than just supplying data, it has the significant advantages
that:
...
`supportsInstances` is a boolean flag that indicates whether the notebook
supports diagrams. When included, the `.Diagram` shape is generated for the logic. See
