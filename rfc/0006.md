...
```ts
import { Aspect, SimpleOlog } from "catcolab-logics/simple-olog";
import { binder } from "catcolab-documents";
...
```
add: from must be an object (was missing); name must be a string (was missing); to must be an object (was missing)
```
...
```
transition: from must be an array (was object)
```
