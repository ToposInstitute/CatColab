...
We generally follow that at runtime invalid inputs:

- Will _throw_ an exception, ideally with data from a `FailureResult`, _if_ it is something Typescript would have caught
...
This very document forms part of a spec we can test against. Our test suites should be complimented by some more detailed tests covering further features and edge cases. If possible we should implement property based tests that show our new API will not cause document corruption.

#### Phase 2: Packages

We will follow on by implementing packages that meet the specs outlined in the test suites.

#### Phase 3: LLM integration

We will use these packages to implement a minimal viable integration of LLM capabilities into CatColab.

#### Phase 4: New features use the API & port existing features.
