...
let
  ...
  backendPortStr = builtins.toString config.catcolab.backendPort;
  automergePortStr = builtins.toString config.catcolab.automergePort;
in
with lib;
{
  options.catcolab = {
  ...
  config = {
    ...
    services.caddy = {
      enable = true;
      virtualHosts = {
        "${config.catcolab.backendHostname}" = {
          extraConfig = ''
            reverse_proxy :${backendPortStr}
          '';
        };

        "${config.catcolab.automergeHostname}" = {
          extraConfig = ''
