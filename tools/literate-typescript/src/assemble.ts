 ...
 *   - `<!-- verifier:skip -->` marks the next code fence so that it is skipped
 *     entirely: it is not materialised, type-checked, or run. Use this for
 *     purely illustrative snippets that reference names not available to the
 *     verifier.
...
export type TsSample = {
    ...
    /** Whether this sample should be skipped entirely (not materialised or run). */
    skip?: boolean;
};
...
export function assemble(items: ParsedItem[], slug: string): Assembled {
    ...
    for (const item of items) {
        ...
        if (skipNext) {
            // Skipped fences are dropped entirely: not materialised, not
            // type-checked, not run, and not added to the prepend stack. Reset
            // the directive flags that would otherwise apply to this fence, and
            // clear `lastTsSample` so a following non-code fence is not attached
            // as expected output.
            skipNext = false;
            prependNext = false;
            throwsNext = false;
