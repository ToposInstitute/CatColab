...
/**
 * Output directories created by {@link prepareOutDir} that have not yet been
 * cleaned up. Tracked so that interruption/crash handlers (see
 * {@link cleanupAllOutDirs}) can remove leftovers the normal `finally` path
 * would otherwise miss.
 */
const activeOutDirs = new Set<string>();
...
export function prepareOutDir(pkgRoot: string, slug: string): string {
    const dir = mkdtempSync(join(pkgRoot, `.literate-typescript-${slug}-`));
    activeOutDirs.add(dir);
    return dir;
}
...
export function cleanupOutDir(dir: string): void {
    ...
    activeOutDirs.delete(dir);
}

/**
 * Remove every output directory still tracked as active. Best-effort: used by
 * the CLI's interruption/crash handlers so `.literate-typescript-*` folders do
 * not leak when the process is killed (SIGINT/SIGTERM) or exits abnormally.
 */
export function cleanupAllOutDirs(): void {
    for (const dir of activeOutDirs) {
        try {
            rmSync(dir, { recursive: true, force: true });
        } catch {
            // Best-effort during teardown; ignore failures.
        }
    }
    activeOutDirs.clear();
}
