...
A literate-TypeScript verifier for `.lts.md` Markdown documents.
...
1. Extracts fenced code blocks (`ts` and `tsx`), honouring these directives:
    ...
    - `<!-- verifier:skip -->` — the next code fence is skipped entirely: it is
      not materialised, type-checked, or run, and is not added to the prepend
      stack. Use this for purely illustrative snippets that reference names not
